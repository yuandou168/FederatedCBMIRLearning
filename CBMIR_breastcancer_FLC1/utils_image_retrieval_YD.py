...
def save_reconstructed_images(y_true, y_pred, run_folders):
    for i in range(0, y_pred.shape[0]):
        ...
        print(original[i])
        output = np.hstack([original,recon])
        cv2.imwrite(run_folders["results_path"] + run_folders["exp_name"] + '/'+run_folders["round_name"] + "/image_" + str(i) + ".png", output)
...
def create_environment(run_folders):
    ...
    try:
        ...
        pass
    try:
        # print("balabala", run_folders["results_path"])
        os.mkdir(run_folders["results_path"])
    except:
        pass

    # Preparing required I/O paths for each experiment -- define exp#### folder
    ...
    # print("EXP", exp_model_folder, exp_res_model)
    ...
    try:
     ...
     # Preparing required I/O paths for each round train experiment -- define round#### folder
    # print("exp_folder", run_folders["exp_name"])
    if len(os.listdir(exp_model_folder)) == 0:
        round_idx = 1
    else:
        round_idx = len(os.listdir(exp_model_folder)) + 1
