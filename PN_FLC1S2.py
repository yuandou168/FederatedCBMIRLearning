...
x_train, x_val, ytrain, ytest = train_test_split(df_pneumo_2d,label2, test_size = 0.05, random_state = 0)
# Saving Training set
...
x_train.to_csv (r'/home/yuandou/test/CBIR/SICAPv2/x_train_FL_C1S2.csv', index = True, header=True) # 90% of x_train_FDL.csv for training
x_val.to_csv (r'/home/yuandou/test/CBIR/SICAPv2/x_val_FL_C1S2.csv', index = True, header=True) # 10% of x_train_FDL.csv for validation
...
class CBIRClient(fl.client.NumPyClient):
    ...
    def get_parameters(self, config):
        ...
        with open(dirctory+'/res_get_paras_client1_scenario2.csv', 'a', encoding='UTF8') as f:
            writer = csv.writer(f)
    ...
    def fit(self, parameters, config):
        ...
        dirctory = run_folders["model_path"] + run_folders["exp_name"] 
        with open(dirctory+'/res_fit_client1_scenario2.csv', 'a', encoding='UTF8') as f:
            writer = csv.writer(f)
